...
const LAT_ROWS = [[18, 51], [30, 65], [42, 90], [50, 101], [55, 107]];
// Everything west of this is cut: 104E in China (Sichuan's edge), 97E north of 43N so Mongolia
// keeps the Khangai and Karakorum.
const WEST_AT = [[40, 104], [43, 97]];
const WEST_LON = 104;   // the China cut, used by the knee
...
const LEFT = [[40, 118.5], [53, 118], [57, 114], [62, 109], [69, 105], [76, 100.5],
              [81, 96.5], [88, 93], [95, 90.5], [112, 90.5]];   // moat edge, row -> column
...
const toTile = (lon, lat) => {
    ...
    const east = lerpTable(EAST_LON, lat), left = lerpTable(LEFT, row), west = lerpTable(WEST_AT, lat);
...
const MAINLAND = [
    ...
    [97.0, 55.0], [97.0, 43.0], [104.0, 40.0],
];
const ISLANDS = {
...
const RANGES = [
    ...
    ["Khangai", 0.35, 1.1, [[97.8, 48.3], [100.0, 47.6], [101.8, 46.8]]],
...
const BIOMES = [
    ["East Asia", "G", 110, undefined, [[96, 17], [147, 17], [147, 56], [96, 56]]],
    ...
    ["Mongolian steppe", "P", 45, undefined, [[96, 42.5], [104, 40.5], [117, 42], [119.5, 44], [119.5, 50], [96, 52]]],
    ["Gobi", "D", 15, undefined, [[96, 42.8], [104, 40.3], [111, 41.8], [113.5, 44], [108, 45.8], [100, 45.2], [96, 44.8]]],
    ["Manchuria", "G", 100, undefined, [[119.5, 41], [135, 42], [135, 50], [119.5, 50]]],
    ["Siberian taiga", "T", 90, 0.7, [[96, 52], [147, 52], [147, 56], [96, 56]]],
];
...
const RESOURCES = [
    ...
    ["Mongolia", 11, ["RESOURCE_HORSES", "RESOURCE_HIDES", "RESOURCE_WOOL", "RESOURCE_SALT", "RESOURCE_CAMELS"], [[96, 41], [119.5, 41], [119.5, 52], [96, 52]]],
    ["Manchuria and the Amur", 11, ["RESOURCE_FURS", "RESOURCE_HARDWOOD", "RESOURCE_HIDES", "RESOURCE_WILD_GAME", "RESOURCE_GOLD", "RESOURCE_IRON"],
        [[119.5, 41], [147, 41], [147, 56], [96, 56], [96, 52], [119.5, 52]]],
];
...
const OCEAN = [
    // Finland's border with Russia, then across the Gulf of Finland's east end: Karelia, Ladoga,
    // Onega and St Petersburg are sea, so Finland (and Scandinavia) stays a landmass of its own and
    // the Gulf of Finland opens into this ocean - the northern sea route from the Baltic to Asia.
